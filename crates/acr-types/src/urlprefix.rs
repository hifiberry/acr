//! Rewriting internal API paths to the externally visible API prefix.
//!
...
///
/// Shared by the rewrite functions above and by the root package's version
/// tagging, which is why it is `pub` rather than private to this module.
pub fn normalize_forwarded_prefix(prefix: Option<&str>) -> Option<String> {
    let raw = prefix?.trim();
    if raw.is_empty() {
        return None;
    }
