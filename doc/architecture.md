...
| `audiocontrol-metadata` | The metadata code: MusicBrainz/TheAudioDB/fanart.tv/Last.fm clients, a token-taking Spotify search client (the Spotify *account* is the player daemon's), cover-art providers, the artist store, the library enricher, its own Rocket routes, its clients of the player daemon (`core_client.rs`, `now_playing_ws.rs`, `library_puller.rs`), the two-stage start-up in `startup.rs`, the `audiocontrol-metadata` daemon's own composition root, and the four CLI tools that only need this crate's code. The daemon binary is here rather than in the root package so that it does not link the player library, and with it ALSA, D-Bus, MPD and evdev. | `musicbrainz.rs`, `lastfm.rs`, `spotify.rs`, `library_enricher.rs`, `core_client.rs`, `now_playing_ws.rs`, `library_puller.rs`, `startup.rs`, `api/`, `src/bin/audiocontrol-metadata.rs`, `src/bin/*.rs` |
