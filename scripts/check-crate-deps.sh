...
# The compiled-in secrets are `pub const`, so their values are inlined at every
# use site and their names appear only as metadata in the crate that declares
# them: `strings ... | grep _OBF` over the player library reads zero whether
# the dependency is present or absent, which makes it evidence of nothing.
# (They are generated by `crates/acr-secrets/build.rs` now, which both daemons
# depend on, so their presence would say nothing about the boundary anyway.)
set -eu
fail=0
...
# The player-only daemon still builds. These are compiles, not graph queries, so
# they are the slow half of the script -- and the only half that would catch a
# metadata call added to main.rs outside a cfg, or an `else` branch that stopped
# compiling.
#
# Two feature sets, because they are different builds and only one of them is
# the one that ships.
#
#   --no-default-features                 the crate boundary on its own: no
#                                         metadata crate and no ALSA, so a
#                                         `#[cfg(not(feature = "metadata"))]`
#                                         branch cannot lean on anything the
#                                         alsa feature happens to bring in.
#   --no-default-features --features alsa exactly what debian/rules builds for
#                                         the package. `alsa` gates real code
#                                         (src/helpers/global_volume.rs,
#                                         src/helpers/volume.rs), so without
#                                         this line the combination that runs
#                                         on every device -- alsa on, metadata
#                                         off -- was compiled nowhere in CI.
#
# The second build reuses the first's dependency graph apart from the alsa
# crates, so it costs a fraction of a full compile.
...
if ! cargo build -p audiocontrol --no-default-features --features alsa --bin audiocontrol >/dev/null 2>&1; then
  echo "the audiocontrol binary does not build with the feature set the package ships; re-run" >&2
  echo "  cargo build -p audiocontrol --no-default-features --features alsa --bin audiocontrol" >&2
  echo "to see why" >&2
  fail=1
fi
exit $fail
