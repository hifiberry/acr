...
pub trait CoverartProvider {
    ...
    fn get_artist_coverart(&self, artist: &str) -> Option<String> {
...
impl CoverartManager {
    ...
    pub fn get_artist_coverart(&self, artist: &str) -> Vec<String> {
        self.providers
            .iter()
            .filter_map(|provider| provider.get_artist_coverart(artist))
            .collect()
    }

    /// Get cover art for a song from all registered providers
    pub fn get_song_coverart(&self, title: &str, artist: &str) -> Vec<String> {
        self.providers
            .iter()
            .filter_map(|provider| provider.get_song_coverart(title, artist))
            .collect()
    }

    /// Get cover art for an album from all registered providers
    pub fn get_album_coverart(&self, title: &str, artist: &str, year: Option<i32>) -> Vec<String> {
        self.providers
            .iter()
            .filter_map(|provider| provider.get_album_coverart(title, artist, year))
            .collect()
    }

    /// Get cover art from a URL from all registered providers
    pub fn get_url_coverart(&self, url: &str) -> Vec<String> {
        self.providers
            .iter()
            .filter_map(|provider| provider.get_url_coverart(url))
            .collect()
    }
